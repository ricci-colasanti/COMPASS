...
```
COMPASS/
├── BlockWorld/                    # Example dataset and config
│   ├── artifical_cencus.csv      # Constraint data
│   ├── artifical_survey.csv      # Microdata records
│   ├── artificial_groups.csv     # Validation groups
│   ├── CheckResults.ipynb        # Jupyter visualization notebook
...
│   └── ...
├── Software/                      # Pre-compiled binaries
│   ├── compass_linux_amd64       # Linux 64-bit
│   ├── compass_windows_amd64.exe # Windows 64-bit
│   ├── compass_darwin_amd64      # macOS Intel
│   └── compass_darwin_arm64      # macOS Apple Silicon
├── img/
│   └── COMPASS.png               # Logo
├── README.md
└── LICENSE
...
Pre-compiled executables are available in the `Software/` directory:

| Platform | Binary | Status |
|----------|--------|--------|
| Linux (64-bit) | `compass_linux_amd64` | ✅ Available |
| Windows | `compass_windows_amd64.exe` | ✅ Available |
| macOS (Intel) | `compass_darwin_amd64` | ✅ Available |
| macOS (Apple Silicon) | `compass_darwin_arm64` | ✅ Available |
...
- **Jupyter Notebook** (for visualization, with numpy, scipy, pandas, matplotlib)

### Quick Start with the BlockWorld Example

The repository includes a complete example dataset in the `BlockWorld/` directory. Follow these steps to run your first simulation:

#### Step 1: Copy the Binary

Navigate to the COMPASS root directory and copy the appropriate binary for your system:

```bash
# For Linux users:
cp Software/compass_linux_amd64 BlockWorld/

# For Windows users (using Command Prompt or PowerShell):
copy Software\compass_windows_amd64.exe BlockWorld\

# For macOS Intel users:
cp Software/compass_darwin_amd64 BlockWorld/

# For macOS Apple Silicon users:
cp Software/compass_darwin_arm64 BlockWorld/
```
...
#### Step 3: Make the Binary Executable (Linux/macOS)

```bash
chmod +x compass_linux_amd64  # or your specific binary name
```
...
echo '{"constraints":"artifical_cencus.csv","microdata":"artifical_survey.csv","groups":"artificial_groups.csv","output":"artificial_synthetic_population.csv","validate":"artificial_synthPopSurvey.csv"}' | ./compass_linux_amd64
```

#### Step 5: View Results

The simulation will generate two output files in the `BlockWorld/` directory:

- **`artificial_synthetic_population.csv`**: Maps area IDs to selected microdata IDs
- **`artificial_synthPopSurvey.csv`**: Compares synthetic fractions against constraint fractions

#### Step 6: Visualize Results with Jupyter

Launch the included Jupyter notebook to visualize and validate the results:

```bash
jupyter notebook CheckResults.ipynb
```

**Required Python packages**: numpy, scipy, pandas, matplotlib, jupyter
...
The notebook will display plots comparing the predicted fractional values for each grouped variable against the corresponding synthetic fractions generated by COMPASS.
...
./compass_linux_amd64 -g
```

### Running with Your Own Data

To use COMPASS with your own data:

1. **Prepare your data files** in the required format (see Data Preparation section)
2. **Create a configuration file** (see Configuration section)
3. **Copy the appropriate binary** to your working directory
4. **Run COMPASS** with your config file

```bash
cp ../Software/compass_linux_amd64 .
./compass_linux_amd64 -f my_config.json
```
...
if __name__ == "__main__":
    config = {
        "constraints" : "artifical_cencus.csv",
        "microdata"   : "artifical_survey.csv",
        "groups"      : "artificial_groups.csv",
        "output"      : "artificial_synthetic_population.csv",
        "validate"    : "artificial_synthPopSurvey.csv",
        "initialTemp": 1000.0,
        "minTemp": 0.001,
        "coolingRate": 0.997,
        "reheatFactor": 0.3,
...
|-----------|------|-------------|---------|
| `constraints` | string | Path to census constraint CSV file | `"artifical_cencus.csv"` |
| `groups` | string | Path to geographical grouping CSV file | `"artificial_groups.csv"` |
| `microdata` | string | Path to survey microdata CSV file | `"artifical_survey.csv"` |
| `output` | string | Path for output synthetic population CSV | `"artificial_synthetic_population.csv"` |
...
The repository includes a test dataset in the `BlockWorld/` directory:
...
The `BlockWorld/config.json` file is pre-configured for the example dataset:
...
```bash
cd BlockWorld
cp ../Software/compass_linux_amd64 .  # or your platform's binary
...
Launch the Jupyter notebook to visualize and validate the results:
...
The notebook will display comparison plots showing how well the synthetic population matches the constraints for each variable group.
...
head -1 artifical_cencus.csv
head -1 artifical_survey.csv
head -1 artificial_groups.csv
```
...
A Jupyter notebook called `CheckResults.ipynb` is included in the `BlockWorld/` directory to visualize COMPASS's performance. It plots the predicted fractional values for each grouped variable alongside the corresponding synthetic fractions.

**Requirements:**
- Python 3.6+ with: numpy, scipy, pandas, matplotlib, jupyter
- Groups file (e.g., `artificial_groups.csv`)
- Results file (e.g., `artificial_synthPopSurvey.csv`)
